...
async function LoadDataBase(conn, m) {
	try {
		if (!global._cachedBotNumber) global._cachedBotNumber = conn.decodeJid(conn.user.id)
...
async function MessagesUpsert(conn, message, store) {
	try {
		if (!global._cachedBotNumber) global._cachedBotNumber = conn.decodeJid(conn.user.id)
		let botNumber = global._cachedBotNumber
		...
		// Hanya proses pesan type 'notify' — blok 'append' (emit ulang internal) supaya tidak double
		if (message.type !== 'notify') return
		// [FIX SELF-MODE] Izinkan owner + bot lolos saat mode SELF
		if (!conn.public && !msg.key.fromMe) {
			const _rawSender = msg.key.participant || msg.key.remoteJid
			const _senderJid = conn.decodeJid(_rawSender)
			const _senderNum = _senderJid?.split('@')[0]
			const _isOwnerOrBot =
				_senderNum === conn.user?.id?.split(':')[0] ||
				(global.owner || []).some(v => v.replace(/[^0-9]/g, '') === _senderNum) ||
				(global.cache?.owner || []).some(v => v.replace(/[^0-9]/g, '') === _senderNum)
			if (!_isOwnerOrBot) return
		}
...
async function Solving(conn, store) {
	if (typeof conn.public === 'undefined') conn.public = true
...
async function Serialize(conn, m, store) {
	...
	if (m.key) {
		...
		if (m.isGroup) {
			// Gunakan global.groupCache jika masih fresh (TTL 5 menit), hindari fetch per pesan
			if (!global.groupCache) global.groupCache = {}
			if (!global.groupCacheTime) global.groupCacheTime = {}
			const _cacheTTL = global.GROUP_CACHE_TTL || 300000
			if (global.groupCache[m.chat] && (Date.now() - (global.groupCacheTime[m.chat] || 0)) < _cacheTTL) {
				m.metadata = global.groupCache[m.chat]
			} else {
				m.metadata = await conn.groupMetadata(m.chat).catch(_ => null)
				if (m.metadata) {
					global.groupCache[m.chat] = m.metadata
					global.groupCacheTime[m.chat] = Date.now()
				}
			}
			m.metadata = m.metadata || { participants: [] }
			m.admins = m.metadata.participants ? m.metadata.participants.filter(e => e.admin !== null).map(e => e.id) : []
	...
	if (m.message) {
		...
		m.timestamp = (typeof m.messageTimestamp === "number" ? m.messageTimestamp : m.messageTimestamp.low ? m.messageTimestamp.low : m.messageTimestamp.high) || (m.msg?.timestampMs || 0) * 1000
